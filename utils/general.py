...
def non_max_suppression(
    ...
    if isinstance(prediction, (list, tuple)):  # YOLOv5 model in validation model, output = (inference_out, loss_out)
        ...
        if prediction.ndim == 3:
            ...
            # YOLO export 常见 [1,C,N]
            if b == 1 and c < n:
                prediction = prediction.permute(0, 2, 1)
                conf = torch.ones((prediction.shape[0], prediction.shape[1], 1), device=prediction.device)
                prediction = torch.cat([
                    prediction[:, :, :4],
                    conf,
                    prediction[:, :, 4:]
                ], dim=2)
                # conf = torch.ones((prediction.shape[0], 1), device=prediction.device)
                # prediction = torch.cat([prediction[:, :4], conf], dim=1)
